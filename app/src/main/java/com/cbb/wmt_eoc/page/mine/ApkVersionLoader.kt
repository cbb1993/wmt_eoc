 ...
 * describe:
 */
class ApkVersionLoader : ObjectLoader() {
    ...
    fun request(map: Map<String, String>): Observable<ApkVersion> {
        val token = SharedPreferencesUtils.readData(StringConstant.TOKEN, "")
        return observe<BaseResponse<ApkVersion>>(
            mScanService.request(token, Constant.apk_version, map)).map(PayLoad<ApkVersion>())
    }

    interface Service {
        @GET
        fun request(@Header("Authorization") token: String, @Url url: String,
                    @QueryMap bean: Map<String, String>): Observable<BaseResponse<ApkVersion>>
    }
}