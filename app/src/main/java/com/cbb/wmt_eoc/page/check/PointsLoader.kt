 ...
 * describe:
 */
class PointsLoader : ObjectLoader() {
    ...
    fun request(map: Map<String, String>): Observable<List<CurrentPointModel>> {
        val token =  SharedPreferencesUtils.readData(StringConstant.TOKEN, "")
        return observe<BaseResponse<List<CurrentPointModel>>>(
            mScanService.request(token, Constant.POINT_CURRENT, map)).map(PayLoad<List<CurrentPointModel>>())
    }

    interface Service {
        @GET
        fun request(@Header("Authorization") token: String, @Url url: String,
                    @QueryMap bean: Map<String, String>): Observable<BaseResponse<List<CurrentPointModel>>>
    }
}